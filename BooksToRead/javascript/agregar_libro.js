...
document.addEventListener("DOMContentLoaded", function () {
    ...
    async function cargarLibroAEditar(libroId){
        try{
            let response = await fetch(`https://667893330bd45250561f25c0.mockapi.io/api/BooksToRead/Libros/${libroId}`)
                if (response.status === 200) {
                    ...
                    formAgregarLibro.elements['estado_lectura'].value = libro.estado_lectura;
                }
            } catch (error) {
